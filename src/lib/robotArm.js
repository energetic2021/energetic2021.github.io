...
export const ROBOT_ARM_MODELS = [
  ...
  { key: 'rebot-arm-hexmovr', label: 'rebot-arm-hexmovr' },
];
...
const ROBOT_ARM_MODEL_ALIASES = {
  ...
  'rebot-arm-7dof': 'rebot-arm-damiao',
  'rebot-arm-lite': 'rebot-arm-robstride',
  'rebot-arm-hexmovr': 'rebot-arm-hexmovr',
  hexmovr: 'rebot-arm-hexmovr',
...
export const REBOT_ARM_HEXMOVR_JOINT_LIMITS = {
  1: { min: -2.61, max: 2.61 },
  2: { min: -3.7, max: 0.0 },
  3: { min: -3.7, max: 0.0 },
  4: { min: -1.57, max: 1.57 },
...
const PROFILE_JOINT_LIMITS = {
  ...
  'rebot-arm-hexmovr': REBOT_ARM_HEXMOVR_JOINT_LIMITS,
};
...
export function defaultFeedbackIdForProfile(armModel, escId) {
  ...
  if (vendor === 'hexmovr') return Number(escId);
...
export function isProfileJointHit(hit, armModel, jointCfg) {
  ...
  if (vendor === 'hexmovr') return fid === Number(jointCfg.esc_id) || fid === 0;
