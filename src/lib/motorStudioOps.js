...
export async function controlMotorOp({
  ...
  try {
    ...
    if (!c.enabled && String(h.vendor) !== 'hexmovr') {
    ...
    if (String(h.vendor) === 'hexmovr') {
      setControls((prev) => ({
        ...prev,
        [motorKey(h)]: { ...(prev[motorKey(h)] || c), enabled: true },
      }));
    }
    return true;
  } catch (e) {
...
export async function zeroMotorOp({
  ...
  const c = controls[motorKey(h)] || defaultControlsForHit(h);
  const vendor = String(h?.vendor || '');
  const isRobstride = vendor === 'robstride';
  const isHexmovr = vendor === 'hexmovr';
  if (!isRobstride && !isHexmovr && !c.enabled) {
  ...
  try {
    ...
    if (isHexmovr) {
      pushLog(`zero ${h.vendor} ${toHex(h.esc_id)} ok`, 'ok');
    } else {
