...
export function defaultControlsForHit(hit) {
  const mode = hit.vendor === 'damiao' || hit.vendor === 'hexmovr' ? 'pos_vel' : (hit.vendor === 'robstride' ? 'mit' : 'vel');
  return {
    mode,
    enabled: false,
    target: 0,
...
export function normalizeHits(vendor, data, model) {
  ...
  for (const h of arr) {
    ...
    const state = h.state && typeof h.state === 'object' ? h.state : {};
    out.push({
      vendor,
      model,
      probe: Number(h.probe ?? h.motor_id ?? h.node_id ?? 0),
      esc_id: Number(h.motor_id ?? h.node_id ?? h.probe ?? 0),
      mst_id: Number(h.feedback_id ?? state.feedback_id ?? 0),
      detected_by: String(h.detected_by ?? ''),
      status: Number(state.status_code ?? h.status ?? Number.NaN),
      pos: Number(state.pos ?? h.pos ?? Number.NaN),
      vel: Number(state.vel ?? h.vel ?? Number.NaN),
      torq: Number(state.torq ?? h.torq ?? Number.NaN),
      t_mos: Number(state.t_mos ?? h.t_mos ?? Number.NaN),
      t_rotor: Number(state.t_rotor ?? h.t_rotor ?? Number.NaN),
      has_value: Boolean(state.has_value ?? h.has_value),
      arbitration_id: Number(state.arbitration_id ?? h.arbitration_id ?? Number.NaN),
      can_id: Number(state.can_id ?? h.can_id ?? Number.NaN),
      updated_at_ms: now,
    });
  }
  return out;
