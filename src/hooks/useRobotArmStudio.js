  ...
  const detectRobotArmModel = async () => {
    ...
    try {
      ...
      if (offlineJoints.length > 0) {
        if (winnerVendor === 'hexmovr') {
          if (winnerProfile !== current) setRobotArmModel(winnerProfile);
          pushLog(
            `robot-arm detect partial: joint 1 is hexmovr, switched profile; offline joints=${offlineJoints.join(',')}`,
            'info'
          );
          markScanDone('robot-arm detect partial');
          return {
            vendor: winnerVendor,
            profile: winnerProfile,
            onlineCount: ROBOT_ARM_JOINTS.length - offlineJoints.length,
