...
如果只想让 Studio 控制 Hexmovr，又不修改 `motorbridge` 的 `ws_gateway`，可以启动本仓库内置的轻量适配网关。它复用已安装的 `motorbridge` Python 包和其中的 `add_hexmovr_motor(...)`：

```bash
npm run gateway:hexmovr
```

等价命令：

```bash
python3 tools/hexmovr_ws_gateway.py --bind 127.0.0.1:9002 --channel can0
```
...
支持 `scan`、`state_once/status/verify`、`enable`、`disable/stop`、`mit`、`pos_vel`、`vel`、`set_zero_position`。Hexmovr 没有独立 enable 帧，`enable` 在适配网关中是兼容空操作，实际运动命令会激活对应控制模式。
